...
// Apply performs an update of targetPath, with the contents of sourcePath.
// When the update fails, it is unlikely that old executable is corrupted,
// but still, applications need to check the returned error with
// RollbackError() and notify the user of the bad news and ask them to recover manually.
func Apply(targetPath string, sourcePath string) error {
	err := prepareAndCheckBinary(targetPath, sourcePath)
	if err != nil {
		return err
	}
	return commitBinary(targetPath)
}
...
func prepareAndCheckBinary(targetPath string, sourcePath string) error {
...
// commitBinary moves the new executable to the location of targetPath. It performs the following operations:
...
func commitBinary(targetPath string) error {
