...
func TestBatcher(t *testing.T) {
	b := NewBatcher()
	b.Add("user", "1", "INSERT", []string{"name"}, map[string]interface{}{"id": "1", "name": "John"})
	...
	b.Add("user", "1", "UPDATE", []string{"name", "age"}, map[string]interface{}{"id": "1", "age": 21, "name": "Foo"})
	assert.NotEmpty(t, b.Records())
	assert.Equal(t, `[{"table":"user","id":"1","operation":"UPDATE","diff":["name","age"],"object":{"age":21,"id":"1","name":"Foo"}}]`, JSONStringify(b.Records()))
	b.Add("user", "1", "UPDATE", []string{"salary"}, map[string]interface{}{"id": "1", "salary": 10, "age": 21, "name": "Foo"})
	assert.NotEmpty(t, b.Records())
