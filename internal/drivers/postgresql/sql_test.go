...
func TestDBChanges(t *testing.T) {
	...
	t.Log(sql)
	assert.Equal(t, "2f0670431ed61e19", util.Hash(sql)) // FIXME: this modelVersion is unstable
...
func TestLedgerUpsertSQL(t *testing.T) {
	assert.Equal(t,
		`INSERT INTO "_eds_row_version" ("table_name","pk","mvcc","updated_at") VALUES ('order','1','V1',now())`+
			` ON CONFLICT ("table_name","pk") DO UPDATE SET "mvcc"=EXCLUDED."mvcc","updated_at"=EXCLUDED."updated_at" WHERE EXCLUDED."mvcc">"_eds_row_version"."mvcc";`+"\n",
...
// ledger upsert; the bulk-import path (empty version) does neither
func TestUpsertGuardedByLedger(t *testing.T) {
	...
	o := map[string]any{"id": "1", "name": "test"}

	unguarded := toSQLFromObject("INSERT", schema, "customer", o, nil, "")
	assert.Equal(t, `INSERT INTO "customer" ("id","name") VALUES ('1','test') ON CONFLICT (id) DO UPDATE SET "name"='test';`+"\n", unguarded)
