...
func TestDBChanges(t *testing.T) {
	...
	const mergeHash = "13d759c08ba832fa"
...
// the bulk-import path (empty version) writes every column on every merge and
// carries no ledger guard
func TestMergeWritesAllColumns(t *testing.T) {
	...
	assert.Equal(t, `MERGE [order] AS target USING (VALUES('1')) AS source ([id]) ON target.[id]=source.[id]`+
...
// the streaming path gates both merge branches on the version ledger and appends
// the ledger upsert so a hard-deleted row cannot be resurrected out of order
func TestMergeGuardedByLedger(t *testing.T) {
	sql := toSQLFromObject(getSmallSchema(), "order", map[string]any{"id": "1", "updatedDate": "2024-07-11T21:16:51.70856Z"}, "V1")
	expected := `MERGE [order] AS target USING (SELECT s.[id], l.[mvcc] AS __mvcc FROM (VALUES('1')) AS s ([id])` +
		` LEFT JOIN [_eds_row_version] l ON l.[table_name]='order' AND l.[pk]='1') AS source ON target.[id]=source.[id]` +
		` WHEN MATCHED AND (source.__mvcc IS NULL OR 'V1'>source.__mvcc) THEN UPDATE SET` +
		` [appointmentDates]='[]',[labels]='{}',[metadata]=NULL,[name]=NULL,[updatedDate]='2024-07-11 21:16:51.70856'` +
		` WHEN NOT MATCHED AND (source.__mvcc IS NULL OR 'V1'>source.__mvcc) THEN INSERT ([id],[appointmentDates],[labels],[metadata],[name],[updatedDate])` +
		` VALUES ('1','[]','{}',NULL,NULL,'2024-07-11 21:16:51.70856');` + "\n" + ledgerUpsertSQL("order", "1", "V1")
	assert.Equal(t, expected, sql)
}

func TestLedgerUpsertSQL(t *testing.T) {
	assert.Equal(t,
		`MERGE [_eds_row_version] AS target USING (VALUES('order','1','V1')) AS source ([table_name],[pk],[mvcc])`+
			` ON target.[table_name]=source.[table_name] AND target.[pk]=source.[pk]`+
			` WHEN MATCHED AND source.[mvcc]>target.[mvcc] THEN UPDATE SET [mvcc]=source.[mvcc],[updated_at]=SYSUTCDATETIME()`+
			` WHEN NOT MATCHED THEN INSERT ([table_name],[pk],[mvcc],[updated_at]) VALUES (source.[table_name],source.[pk],source.[mvcc],SYSUTCDATETIME());`+"\n",
		ledgerUpsertSQL("order", "1", "V1"))
}
