...
func TestDBChanges(t *testing.T) {
	registry, err := registry.NewAPIRegistry(context.Background(), logger.NewTestLogger(), "http://api.shopmonkey.cloud", "test", nil)
	...
	assert.NoError(t, err)
	t.Log(sql)
	assert.Equal(t, "94dfea5db940628a", util.Hash(sql)) // FIXME this modelVersion is unstable
