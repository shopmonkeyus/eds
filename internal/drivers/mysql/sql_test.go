...
func TestDBChanges(t *testing.T) {
	...
	t.Log(sql)
	assert.Equal(t, "4d3a975c1c9045b7", util.Hash(sql)) // FIXME this modelVersion is unstable
...
func TestLedgerUpsertSQL(t *testing.T) {
	assert.Equal(t,
		"INSERT INTO `_eds_row_version` (`table_name`,`pk`,`mvcc`,`updated_at`) VALUES ('order','1','V1',NOW())"+
			" ON DUPLICATE KEY UPDATE `mvcc`=IF(VALUES(`mvcc`)>`mvcc`,VALUES(`mvcc`),`mvcc`),`updated_at`=IF(VALUES(`mvcc`)>`mvcc`,VALUES(`updated_at`),`updated_at`);\n",
...
// DUPLICATE KEY UPDATE and appends the ledger upsert; the bulk-import path (empty
// version) stays unguarded
func TestUpsertGuardedByLedger(t *testing.T) {
	...
	event := internal.DBChangeEvent{After: json.RawMessage(`{"id":"1","name":"test"}`)}

	unguarded, err := toSQLFromObject("INSERT", schema, "customer", event, nil, "")
	assert.NoError(t, err)
	assert.Equal(t, "INSERT INTO `customer` (`id`,`name`) VALUES ('1','test') ON DUPLICATE KEY UPDATE `name`='test';\n", unguarded)

	guarded, err := toSQLFromObject("INSERT", schema, "customer", event, nil, "V1")
	assert.NoError(t, err)
	assert.Equal(t,
		"INSERT INTO `customer` (`id`,`name`) SELECT '1','test' FROM DUAL WHERE NOT "+ledger.NotNewer("customer", "1", "V1")+
